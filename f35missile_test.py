...
f35 = Entity(model=f35, position = (0,16.5,0), scale=(0.84, 0.84, 0.84), collider='box')
cube = Entity()
f35.parent = cube
missile1.parent = cube
missile2.parent = cube
missile1.rotation_y = 90
missile2.rotation_y = 90
...
def update():
    ...
    missile1.position -= missile1.right * missile_speed * time.dt * 100
    # roll_decay = 60
    ...
    cube.position -= cube.right * engine_speed * time.dt
    ...
    if held_keys['l']:
        missile_speed += 1 * time.dt * 10
        missile1.parent = None
        # missile_speed = 100
